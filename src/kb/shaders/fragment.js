...
float t = (uTime) + step(length(uv-.5), .5);
float t2 = (uTime * .8) + uv.x;
float t3 = (uTime *.7) + uv.y;
float t4 = (uTime *.5) + step(length(uv), .8);
...
vec2 coOrd = gl_PointCoord;
  ...
  vec2 roteCU = rotateUV(uv2, vec2(.5), -PI * uTime * .05);
  

  vec2 roteU = rotateUV(uv2, vec2(.5), PI * uTime * .05);
     
        color = vec3( 0., 0., 1.);
        vec3 color2 = vec3( 0.2, 0.5, 1.);
   ...
   if(index == 0.0){
    ...
    if(index == 1.0){
        ...
        alpha = 1.- step(shape(roteC, 30., .2),.5);
        ...
        alpha = 1.- step(shape(rote, 4., .3),.5);

     
    } else if(index == 3.0){
        //  Rotate cell 3 by 180 degrees

        alpha = 1.- step(shape(roteC, 40., .3),.5);
    }
 



color = mix(color, color2, step(cnoise(roteU * 2. * step(cnoise(roteCU * 3.), uv.x)) , .1));

color = mix(color, color+ .15, step(cnoise(roteCU * 3. * step(cnoise(roteU * 2.), uv.y)) , .1));
