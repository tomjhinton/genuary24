...
void main() {
  ...
  uv = rotateTilePattern(uv * uv.y);
  uv = rotateTilePattern(uv);


  uv = vec2(uv.x + sin(uv.y *( 5.0 * sin( (uTime * .2) + uvS.x))) * 0.8,
  uv.y + sin(uv.x * (5.0 * cos((uTime * .2) +uvS.y))) * 0.4);


  // uv = getRadialUv(uv-.5);

vec3 color = vec3(shape(uv,30. * uvS.x, .3) - shape(uv,40. * uvS.y, .15));
uv = uvS;
