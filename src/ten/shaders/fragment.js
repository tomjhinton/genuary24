...
void main() {
 ...
 vec3 color = vec3(1., pos.z, 0.2);

 color = mix(color, vec3(1., 0., 0.),  1.- step(shape(coOrd, 4., .4 ), .3)) ;

 color = mix(color, vec3(1., 0.5, pos.z),  1.- step(shape(coOrd, 4., .2 ), .3)) ;

//  color = mix(color, vec3(uv.x,uv.y, 1.),  1.- step(shape(coOrd, 4., .1 ), .3)) ;
   ...
   alpha = 1.- step(shape(coOrd, 4., pos.z ), .1) ;
